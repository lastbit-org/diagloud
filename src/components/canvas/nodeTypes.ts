import {
  ArtifactNode,
  BuildNode,
  KmsNode,
  BigqueryNode,
  SpannerNode,
  FirestoreNode,
  BigtableNode,
  FirebaseNode,
  WorkbenchNode,
  NotebookNode,
  SparkNode,
  AirflowNode,
  DataflowNode,
  ModelRegistryNode,
  TuningNode,
  EvaluationNode,
  EndpointsNode,
  BatchInferenceNode,
  FeatureStoreNode,
  ExperimentsNode,
  TrainingNode,
  PipelinesNode,
  MlMonitoringNode,
  GkeNode,
  InternetNode,
  NatNode,
  RouterNode,
  PeeringNode,
  VpnNode,
  InterconnectNode,
  FirewallNode,
  DnsNode,
  PubsubNode,
  EventarcNode,
  RunNode,
  SqlNode,
  StorageNode,
  SubnetNode,
  VmNode,
  InstanceGroupNode,
  VpcNode,
  ZoneNode,
  FolderNode,
  ProjectNode,
  InfocardNode,
  EntraNode,
  PcUserNode,
  OnpremNode,
  GithubNode,
  AzDoRepoNode,
  AzDoPipelineNode,
  IamNode,
  LoadBalancerNode,
  CdnNode,
  OrgPolicyNode,
  PscNode,
  SecretManagerNode,
  CertificateManagerNode,
  ApigeeNode,
  MemorystoreNode,
  AlloydbNode,
  CloudShellNode,
  MonitoringNode,
  CloudLoggingNode,
  CloudArmorNode,
  KnowledgeCatalogNode,
  UserGroupNode,
} from "../nodes";

export const nodeTypes = {
  vpc: VpcNode,
  subnet: SubnetNode,
  vm: VmNode,
  instancegroup: InstanceGroupNode,
  storage: StorageNode,
  sql: SqlNode,
  gke: GkeNode,
  nat: NatNode,
  router: RouterNode,
  peering: PeeringNode,
  vpn: VpnNode,
  interconnect: InterconnectNode,
  firewall: FirewallNode,
  dns: DnsNode,
  artifact: ArtifactNode,
  build: BuildNode,
  kms: KmsNode,
  internet: InternetNode,
  run: RunNode,
  pubsub: PubsubNode,
  eventarc: EventarcNode,
  bigquery: BigqueryNode,
  spanner: SpannerNode,
  firestore: FirestoreNode,
  bigtable: BigtableNode,
  firebase: FirebaseNode,
  workbench: WorkbenchNode,
  notebook: NotebookNode,
  spark: SparkNode,
  airflow: AirflowNode,
  dataflow: DataflowNode,
  modelregistry: ModelRegistryNode,
  tuning: TuningNode,
  evaluation: EvaluationNode,
  endpoints: EndpointsNode,
  batchinference: BatchInferenceNode,
  featurestore: FeatureStoreNode,
  experiments: ExperimentsNode,
  training: TrainingNode,
  pipelines: PipelinesNode,
  mlmonitoring: MlMonitoringNode,
  zone: ZoneNode,
  folder: FolderNode,
  project: ProjectNode,
  infocard: InfocardNode,
  entra: EntraNode,
  pcuser: PcUserNode,
  onprem: OnpremNode,
  github: GithubNode,
  azdorepo: AzDoRepoNode,
  azdopipeline: AzDoPipelineNode,
  iam: IamNode,
  loadbalancer: LoadBalancerNode,
  cdn: CdnNode,
  orgpolicy: OrgPolicyNode,
  psc: PscNode,
  secretmanager: SecretManagerNode,
  certificatemanager: CertificateManagerNode,
  apigee: ApigeeNode,
  memorystore: MemorystoreNode,
  alloydb: AlloydbNode,
  cloudshell: CloudShellNode,
  monitoring: MonitoringNode,
  cloudlogging: CloudLoggingNode,
  cloudarmor: CloudArmorNode,
  knowledgecatalog: KnowledgeCatalogNode,
  usergroup: UserGroupNode,
};
